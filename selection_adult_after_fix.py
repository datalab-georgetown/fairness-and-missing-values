...
class AdultDataset_train(StandardDataset):
    ...
    def __init__(
        ...
        _, df_neg_priv_test = train_test_split(
            df_neg_priv, test_size=4650, random_state=25)
        _, df_neg_unpriv_test = train_test_split(
            df_neg_unpriv, test_size=2850, random_state=25)
        ...
        _, df_pos_priv_test = train_test_split(
            df_pos_priv, test_size=1800, random_state=25)
        _, df_pos_unpriv_test = train_test_split(
            df_pos_unpriv, test_size=700, random_state=25)
        ...
        for i in df['income-per-year'].unique():
            for j in df['sex'].unique():
                orig_df = df.loc[(df['income-per-year'] == i)
                                 & (df['sex'] == j), :]
                real_count = len(orig_df.index)
                exp_count = int((len(df.loc[(df['income-per-year'] == i), :].index) / len(
